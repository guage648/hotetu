...
// =======================================================================
// このセクションのコンポーネントコードは変更ありませんので、
// 簡潔にするため、変更があったメインアプリケーション部分のみを記載します。
// Icon, SearchBar, PromoCarouselなどのコンポーネントはそのままお使いください。
...
function HotelDetailView({ hotel, onClose }) {
  ...
  const handleNavigation = () => {
    // 優先使用經緯度，如果沒有，則使用飯店名稱
    // URL Scheme格式: amapuri://route/plan/?dlat=纬度&dlon=经度&dname=目的地名称&dev=0&t=0
    ...
    >
      ...
      >
        <div style={{ flex: 1, paddingRight: "16px" }}>
          <h3 style={{ margin: "16px 0 4px 0", fontSize: "1.1rem" }}>
            {hotel.name}
          </h3>
          <p
            style={{
