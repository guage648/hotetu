    ...
    return (
        <div ref={searchContainerRef} style={{ position: "relative", padding: "16px 16px 8px 16px", background: COLORS.bg, zIndex: 100 }}>
            <div style={{ position: "relative" }}>
                ...
                <input type="text" value={query} onChange={handleQueryChange} placeholder="ホテル名で検索..." style={{ width: "100%", padding: "10px 10px 10px 40px", borderRadius: "20px", border: `1px solid ${COLORS.cardBorder}`, boxSizing: "border-box", fontSize: "1rem" }} />
            </div>
            {results.length > 0 && (
                <ul style={{ position: "absolute", top: "90%", left: 16, right: 16, background: COLORS.card, border: `1px solid ${COLORS.cardBorder}`, borderRadius: "12px", listStyle: "none", padding: "8px 0", margin: 0, zIndex: 1000, boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}>
                    {results.map((hotel) => (
...
function FunctionButtons() {
    const buttons = [ { label: "ブティック", icon: "boutique" }, { label: "ファミリー", icon: "family" }, { label: "ラグジュアリー", icon: "luxury" }, { label: "特別セール", icon: "deals" }, ];
    return (
    ...
    return (
        <div className="destination-filter" style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 16, marginBottom: 0 }}>
            {DESTINATIONS.map((dest) => (
                <button key={dest.id} onClick={() => onSelect(dest.id)} style={{ padding: "8px 16px", border: "none", borderRadius: 20, background: selectedId === dest.id ? COLORS.accent : COLORS.card, color: selectedId === dest.id ? "white" : COLORS.text, cursor: "pointer", flexShrink: 0, boxShadow: "0 2px 8px rgba(0,0,0,0.05)", transition: "all 0.2s ease", fontWeight: selectedId === dest.id ? "bold" : "normal" }}>
                    {dest.name}
                </button>
            ))}
        </div>
...
function HotelCard({ hotel, onSelect }) {
    return (
        <div onClick={() => onSelect(hotel.id)} style={{ background: COLORS.card, borderRadius: 16, boxShadow: "0 4px 12px rgba(0,0,0,0.05)", overflow: "hidden", cursor: "pointer", display: "flex", flexDirection: "column" }}>
            <div style={{ height: 120, backgroundImage: `url(${hotel.cover_image_url})`, backgroundSize: "cover", backgroundPosition: "center", backgroundColor: COLORS.cardBorder }} />
            <div style={{ padding: "8px", flex: 1, display: "flex", flexDirection: "column" }}>
                <h3 style={{ fontSize: "0.9rem", fontWeight: "600", margin: "0 0 4px 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{hotel.name}</h3>
                <div style={{ fontSize: "0.75rem", color: COLORS.textSecondary, margin: "2px 0" }}>{hotel.distance_text}</div>
                <div style={{ display: "flex", alignItems: "center", gap: 4, margin: "4px 0" }}>
                    <Icon name="star" size={14} style={{ color: COLORS.star }} /> <span style={{ fontWeight: "bold", fontSize: "0.8rem" }}>{hotel.rating_score}</span> <span style={{ fontSize: "0.7rem", color: COLORS.textSecondary }}>({hotel.review_count}件)</span>
                </div>
                <div style={{ textAlign: "left", marginTop: "auto", paddingTop: "4px" }}>
                    <span style={{ fontSize: "1rem", fontWeight: "bold", color: COLORS.accent }}>{hotel.price_jpy.toLocaleString()}円</span> <span style={{ fontSize: "0.7rem", color: COLORS.textSecondary }}>/泊~</span>
                </div>
            </div>
        </div>
    );
...
function MainPage({ allHotels, displayHotels, onSelectHotel, onShowFilters, sort, setSort, selectedDestinationId, onDestinationChange, promoImages, startDate, setStartDate, endDate, setEndDate }) {
    return (
...
// NOTE: Modal components are simplified for brevity. Use your full modal code.
const FilterModal = ({ onClose, onApply }) => { return <div style={{position:'fixed', zIndex: 3000, inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center'}}><div style={{background:'white', padding:20, borderRadius:8}}>Filter Modal<button onClick={onApply}>Apply</button><button onClick={onClose}>Close</button></div></div> };
const BookingModal = ({ room, hotelName, onClose }) => { return <div style={{position:'fixed', zIndex: 2000, inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center'}}><div style={{background:'white', padding:20, borderRadius:8}}>Booking Modal for {room.name}<button onClick={onClose}>Close</button></div></div> };
const ReviewModal = ({ hotel, onClose }) => { return <div style={{position:'fixed', zIndex: 2000, inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center'}}><div style={{background:'white', padding:20, borderRadius:8}}>Review Modal for {hotel.name}<button onClick={onClose}>Close</button></div></div> };
const VRConnectionModal = ({ onClose }) => { return <div style={{position:'fixed', zIndex: 10001, inset:0, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center'}}><div style={{background:'white', color: 'black', padding:'20px 40px', borderRadius:12, textAlign:'center'}}><h3>VR接続</h3><p>VRヘッドセットを接続してください...</p><button onClick={onClose} style={{marginTop: 10, padding: '8px 16px'}}>閉じる</button></div></div> };

// ===== 修正点2: 新しいフローティングVRボタン =====
    ...
    return (
        <div style={{ background: COLORS.bg, minHeight: "100vh", animation: "slideInUp 0.3s" }}>
            <header style={{ position: "sticky", top: 0, zIndex: 10, background: "rgba(255,255,255,0.8)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", padding: "12px", borderBottom: `1px solid ${COLORS.cardBorder}` }}>
                <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer" }}><Icon name="back" size={24} /></button>
                <h2 style={{ margin: "0 auto", transform: "translateX(-12px)", fontSize: "1.1rem", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", padding: "0 10px" }}>{hotel.name}</h2>
            </header>
            ...
            <div style={{ padding: "0 16px 80px 16px" }}> {/* Add padding-bottom to avoid overlap with floating button */}
                <p>客室選択、施設詳細、アメニティ情報...</p>
                 {/* You can map and display hotel.rooms here */}
            </div>

            {/* ===== 修正点2: フローティングVRボタンをここに追加 ===== */}
            {/* この行で、詳細ページにフローティングボタンを描画しています */}
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const formattedData = hotelJson.map((h, index) => {
                    // ===== 修正点1: データマッピングの再確認 =====
                    // ここでスプレッドシートの列名（中国語）をJavaScriptオブジェクトのキーに正しくマッピングします
                    const priceJpy = parseFloat(h["価格1"]) || 0;
                ...
                }).filter(h => h.name); // 空の行を除外

                setAllHotels(formattedData);
                const priceRange = { min: Math.floor(minPrice / 1000) * 1000, max: Math.ceil(maxPrice / 1000) * 1000 };
    ...
    useEffect(() => {
        ...
        result = result.filter(
            (h) =>
                ...
                h.rooms && h.rooms.length > 0 && h.rooms.some((room) => room.maxOccupancy >= guestCount)
        );
        switch (sort) {
            ...
            default: result.sort((a, b) => (b.rating_score * Math.log10(b.review_count + 2)) - (a.rating_score * Math.log10(a.review_count + 2))); break;
        }
    ...
    return (
        <div style={{ background: COLORS.bg, color: COLORS.text, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', minHeight: "100vh" }}>
            ...
            <div style={{ display: selectedHotel ? "none" : "block" }}>
                <MainPage {...{ allHotels, displayHotels, onSelectHotel: setSelectedHotelId, onShowFilters: () => setShowFilters(true), sort, setSort, selectedDestinationId, onDestinationChange: setSelectedDestinationId, promoImages, startDate, setStartDate, endDate, setEndDate }} />
            </div>
            {selectedHotel && <HotelDetailView hotel={selectedHotel} onClose={() => setSelectedHotelId(null)} />}
            {showFilters && <FilterModal {...{ onClose: () => setShowFilters(false), onApply: () => setShowFilters(false), filters, setFilters, priceBounds, availableAmenities, guestCount, onGuestChange: setGuestCount }} />}
        </div>
    );
}
